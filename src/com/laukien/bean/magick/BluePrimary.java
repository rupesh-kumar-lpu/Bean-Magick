 ...
 *
 */
public class BluePrimary extends CommandAbstract {
	private int gLeft;
	private int gTop;
	...
	public void setPoint(String pPoint) {
		if(com.laukien.string.String.isEmpty(pPoint)) throw new ParameterException("BluePrimary.setPoint: Invalid format");
		try {
			...
			if(pos==-1) {
				...
				if(pos==-1) {
					pos=pPoint.indexOf(',');
					if(pos==-1) throw new ParameterException("BluePrimary.setPoint: Invalid format");
				}
			}
			...
			throw new ParameterException("BluePrimary.setPoint: Invalid format");
		}
	}
	...
	public String getStatement() {
		...
		else return "-blue-primary "+getPoint();
	}
