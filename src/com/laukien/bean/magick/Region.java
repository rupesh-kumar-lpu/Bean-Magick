 ...
 *
 */
public class Region extends CommandAbstract {
	private int gWidth;
	private int gHeight;
	private int gLeft;
	private int gTop;
	...
	public void setGeometry(String pGeometry) {
		if(com.laukien.string.String.isEmpty(pGeometry)) throw new ParameterException("Region.setGeometry: Invalid format");
		try {
			...
			if(pos==-1) {
				...
				if(pos==-1) {
					...
					if(pos==-1) throw new ParameterException("Region.setGeometry: Invalid format");
				}
			}
			...
			throw new ParameterException("Region.setGeometry: Invalid format\n"+pGeometry);
		}
	}
